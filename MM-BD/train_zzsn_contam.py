...
parser.add_argument('--attack_dir', default='attack_zzsn',
                    type=str, help='output direction')
parser.add_argument('--model_dir', default='model_zzsn',
...
# delete classes higher than 5

ind_to_delete = [i for i,label in enumerate(trainset.targets) if label>=4]
trainset.data = np.delete(trainset.data, ind_to_delete, axis=0)
trainset.targets = np.delete(trainset.targets, ind_to_delete, axis=0)

ind_to_delete = [i for i,label in enumerate(testset.targets) if label>=4]
testset.data = np.delete(testset.data, ind_to_delete, axis=0)
testset.targets = np.delete(testset.targets, ind_to_delete, axis=0)
...
classes = ('plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck')[:4]
...
def test_attack(epoch):
    ...
    total = 0
    with torch.no_grad():
        for batch_idx, (inputs, targets) in tqdm(enumerate(attackloader)):
            inputs, targets = inputs.to(device), targets.to(device)
            outputs = net(inputs)
